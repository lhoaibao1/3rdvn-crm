...
class ProjectWorkflowConfiguration
{
    ...
    public static function defaults(?string $projectSlug): array
    {
        return match ($projectSlug) {
            'acl-mix' => [
                self::step(AclMixWorkflow::PENDING_INITIAL_REVIEW, 'Chờ kiểm tra', [AclMixWorkflow::INELIGIBLE, AclMixWorkflow::OTP_REQUIRED, AclMixWorkflow::RETURNED_TO_SALE], self::SPECIAL, 'Người xử lý chọn Không thoả điều kiện, Yêu cầu OTP hoặc Trả về Sale.'),
                self::step(AclMixWorkflow::OTP_REQUIRED, 'Đang kiểm tra', [AclMixWorkflow::CUSTOMER_CAPP, AclMixWorkflow::RETURNED_TO_SALE], self::SPECIAL, 'Cập nhật OTP nhưng giữ nguyên trạng thái; có thể chuyển CAPP hoặc Trả về Sale.'),
                self::step(AclMixWorkflow::CUSTOMER_CAPP, 'Khách hàng thao tác CAPP', [AclMixWorkflow::SALE_COMPLETION, AclMixWorkflow::REJECTED, AclMixWorkflow::RETURNED_TO_SALE], self::MANUAL, 'Khách hàng thoả mãn điều kiện tiếp tục workflow cũ; có thể Từ chối hoặc Trả về Sale.'),
