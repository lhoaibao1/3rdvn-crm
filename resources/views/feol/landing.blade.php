    ...
    <section class="card">
        ...
        <form method="post" action="{{ route('feol.landing.store', ['token' => $integration->public_token]) }}" id="feol-form">
            ...
            <div class="grid">
                ...
                <div class="field"><label>Mã giới thiệu</label><input value="{{ data_get($application->payload, 'fields.referral_code') }}" readonly></div>
                <div class="field"><label>Mã nhân viên</label><input value="{{ data_get($application->payload, 'fields.salesman_code') }}" readonly></div>
